...
public class ImageContent {
  ...
  public static ImageContent prepare(
      OpenShift openShift, String imageUrl, Map<String, String> envs) {
  ...
  public static ImageContent prepare(
      OpenShift openShift, String imageUrl, String name, List<String> command) {
  ...
  private static Pod getPod(
      String imageUrl, String name, List<String> command, Map<String, String> envs) {
    ...
    // Setting security context as required to comply with "restricted" pod security standards
    // (since OCP 4.12/Kubernetes 1.25)
  ...
  /**
   * Will return maven version in format X.Y.Z. In RHEL7, maven needs to be added to path via enable
   * script. This is a default behaviour of this method. Behaviour can be changed by changing
   * XTFConfig properties.
   *
   * <ul>
   *   <li><i>xtf.maven.activation_script</i> - set to path to maven enable script. Default: <code>
   *       /opt/rh/rh-maven35/enable</code>
   * </ul>
   *
  ...
  public String mavenVersion() {
    ...
    return shell
        .executeWithBash(mavenScriptPath)
        .getOutput()
        .replaceAll("\n", "")
  ...
  private String getMavenActivationScript(String mavenScriptPath) {
    String mavenEnableScript =
        XTFConfig.get("xtf.maven.activation_script", "/opt/rh/rh-maven35/enable");
