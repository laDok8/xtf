...
public class BasicOpenShiftTest {
  ...
  private static final String TEST_RESOURCE_LABEL_VALUE_APP =
      "xtf-core-test-openshift-mocked-smoke";
  ...
  public BasicOpenShiftTest() {
    ...
    // we want to test the XTF OpenShift client, but we need to configure it with the mocked server
    // client
    // properties
    OpenShiftClient mockedServerClient = openShiftServer.getOpenshiftClient();
    this.openShiftClient =
        OpenShift.get(
  ...
  public void testConfigMapBuilder() {
    ...
    final ConfigMap configMap =
        new ConfigMapBuilder(TEST_CONFIGMAP_NAME)
    ...
    Assert.assertEquals(
        String.format(
            "ConfigMap resource has unexpected data entry value: %s", dataEntry.getValue()),
        dataEntryValue,
        dataEntry.getValue());
  }
  ...
  public void testDeploymentConfigBuilder() {
    ...
    final DeploymentConfig deploymentConfig =
        new DeploymentConfigBuilder(TEST_DEPLOYMENT_CONFIG_NAME)
            .addLabels(TEST_RESOURCE_LABELS)
    ...
    // assert
    List<DeploymentConfig> actualResources =
        openShiftClient
            .deploymentConfigs()
    ...
    Assert.assertEquals(
        String.format(
            "DeploymentConfig resource list has unexpected size: %d.", actualResources.size()),
        1,
    ...
    Assert.assertNotNull(
        String.format("DeploymentConfig resource has null \".spec\"", actualResource.getSpec()));
    Assert.assertEquals(
        String.format(
