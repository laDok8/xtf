...
public class ContainerBuilder implements EnvironmentConfiguration, ResourceLimitBuilder {
  ...
  public ContainerBuilder addVolumeMount(
      String name, String mountPath, boolean readOnly, String subPath) {
  ...
  public Container build() {
    ...
    Stream<EnvVar> definedVars =
        envVars.entrySet().stream()
            .map(entry -> new EnvVar(entry.getKey(), entry.getValue(), null));
    Stream<EnvVar> referredVars =
        referredEnvVars.entrySet().stream()
            .map(
                entry ->
                    new EnvVar(
                        entry.getKey(),
    ...
    if (preStopHandler != null) {
      builder.withNewLifecycle().withPreStop(preStopHandler.build()).endLifecycle();
    }
    ...
    final List<ComputingResource> requests =
        computingResources.values().stream()
            .filter(x -> x.getRequests() != null)
            .collect(Collectors.toList());
    final List<ComputingResource> limits =
        computingResources.values().stream()
            .filter(x -> x.getLimits() != null)
            .collect(Collectors.toList());
    if (!requests.isEmpty() || !limits.isEmpty()) {
      ResourcesNested<io.fabric8.kubernetes.api.model.ContainerBuilder> resources =
      ...
      if (!requests.isEmpty()) {
        resources.withRequests(
            requests.stream()
                .collect(
                    Collectors.toMap(
                        ComputingResource::resourceIdentifier,
                        x -> new Quantity(x.getRequests()))));
      }
      if (!limits.isEmpty()) {
  ...
  public ContainerBuilder configFromConfigMap(
      String configMapName, Collection<String> configMapKeys) {
    return configFromConfigMap(configMapName, Function.identity(), configMapKeys);
  }

  public ContainerBuilder configFromConfigMap(
      String configMapName, Function<String, String> nameMapping, String... configMapKeys) {
  ...
  public ContainerBuilder configFromConfigMap(
    ...
    configMapKeys.forEach(
        x -> referredEnvVars.put(nameMapping.apply(x), new Entry(configMapName, x)));
    return this;
  }
