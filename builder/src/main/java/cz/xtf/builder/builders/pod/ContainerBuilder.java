...
public class ContainerBuilder implements EnvironmentConfiguration, ResourceLimitBuilder {
    ...
    public Container build() {
        ...
        Stream<EnvVar> definedVars =
                envVars.entrySet().stream().map(entry -> new EnvVar(entry.getKey(), entry.getValue(), null));
        Stream<EnvVar> referredVars = referredEnvVars.entrySet().stream()
                .map(entry -> new EnvVar(
                        entry.getKey(),
                        null,
                        new EnvVarSource(
        ...
        builder.withVolumeMounts(volumeMounts.stream()
                .map(item -> new VolumeMountBuilder()
    ...
    public ContainerBuilder configFromConfigMap(
            String configMapName, Function<String, String> nameMapping, Collection<String> configMapKeys) {
