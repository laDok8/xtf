...
public class DeploymentConfigBuilder
  ...
  public DeploymentConfig build() {
    ...
    if (imageChangeTrigger) {
      podBuilder.getContainers().stream()
          .forEach(
              container -> {
                ObjectReferenceBuilder imageRef =
    ...
    if (configurationChangeTrigger) {
      triggers.add(new DeploymentTriggerPolicyBuilder().withType("ConfigChange").build());
    }
    if (manualTrigger) {
    ...
    DeploymentConfigSpecBuilder spec =
