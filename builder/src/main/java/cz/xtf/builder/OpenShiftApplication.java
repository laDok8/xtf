...
public class OpenShiftApplication {
  ...
  private void createResources() {
    ...
    serviceAccounts =
        serviceAccounts.stream().map(openShift::createServiceAccount).collect(Collectors.toList());
    imageStreams =
        imageStreams.stream().map(openShift::createImageStream).collect(Collectors.toList());
    buildConfigs =
        buildConfigs.stream().map(openShift::createBuildConfig).collect(Collectors.toList());
    persistentVolumeClaims =
        persistentVolumeClaims.stream()
    ...
    final List<DeploymentConfig> syncDeployments =
        deploymentConfigs.stream()
            .filter(
                x ->
                    x.getMetadata()
                        .getLabels()
                        .containsKey(DeploymentConfigBuilder.SYNCHRONOUS_LABEL))
            .sorted(
                (dc1, dc2) -> {
                  final int labelDc1 =
                      Integer.parseInt(
            ...
            .map(
                x -> {
                  final String syncId =
                      x.getMetadata().getLabels().get(DeploymentConfigBuilder.SYNCHRONOUS_LABEL);
                  final DeploymentConfig dc = openShift.createDeploymentConfig(x);

                  if (dc.getSpec().getReplicas() > 0) {
                    try {
                      log.info(
                          "Waiting for a startup of pod with deploymentconfig '{}' ({} {})",
                      ...
                      openShift
                          ...
                          .areExactlyNPodsReady(
                              dc.getSpec().getReplicas(), dc.getMetadata().getName())
                          .waitFor();
                    } catch (Exception e) {
                      throw new IllegalStateException(
                          "Timeout while waiting for deployment of " + dc.getMetadata().getName(),
                          e);
                    }
                  }
                  return dc;
    ...
    autoScalers =
        autoScalers.stream()
            ...
            .collect(Collectors.toList());
    roles =
        roles.stream().map(r -> openShift.rbac().roles().create(r)).collect(Collectors.toList());
    roleBindings =
        roleBindings.stream().map(openShift::createRoleBinding).collect(Collectors.toList());
  }
}
