...
/**
 * Test cases to verify the SLS configuration and usage through system property, i.e. {@code
...
public class SystemPropertyBasedServiceLogsConfigurationTest {
  ...
  public static void verifyOneConfigurationIsCreatedForAGivenTarget(
    ...
    List<ServiceLogsSettings> configurationsTargeting =
        configurations.list().stream()
    ...
    Assertions.assertEquals(
        ...
        String.format(
            "The number of SLS configurations loaded at runtime (%d) and targeting \"%s\" does not"
                + " equal the expected number of property based SLS configurations (%d)",
  ...
  public static void verifyExactNumberOfConfigurationsCreated(
    ...
    Assertions.assertEquals(
        ...
        actual,
        String.format(
            "The number of SLS configurations loaded at runtime (%d) does not equal the expected"
                + " number of property based SLS configurations (%d)",
            actual, expected));
  }

  /**
   * Check that the correct number of configurations are created, and also that the expected
   * configurations exists, based on the system property value.
   */
  @Test
  public void testCorrectSlsConfigurationNumberIsCreated() {
    ...
    // let's check we have the exact number of SLS configs that we expect from the property we
    // provided
  ...
  /**
   * Check that SLS configuration attributes are properly mapped when using the system property as a
  ...
  /**
   * Check that a SLS configuration which has a {@code output} attribute set to a *Nix file path is
   * accepted
   */
  @Test
  ...
  /**
   * Check that a SLS configuration which has a {@code output} attribute set to a Windows-like file
   * path is accepted
   */
  @Test
  ...
  /**
   * Check that a SLS configuration which has a {@code target} attribute set to a regex is accepted
   */
  @Test
