...
public class Https {
    ...
    Supplier<String> getContent =
        () -> {
          try {
            ...
            log.warn(
                "Attempt to retrieve http content from {} has failed. ({})", url, e.getMessage());
            return "";
          }
        };

    return new SupplierWaiter<>(
        getContent, content -> Stream.of(predicates).allMatch(p -> p.test(content)));
  }
  ...
  public static void copyHttpsURLToFile(
      String source, File destination, int connectionTimeout, int readTimeout) throws IOException {
    copyHttpsURLToFile(Https.urlFromString(source), destination, connectionTimeout, readTimeout);
  }

  public static void copyHttpsURLToFile(
      URL source, File destination, int connectionTimeout, int readTimeout) throws IOException {
