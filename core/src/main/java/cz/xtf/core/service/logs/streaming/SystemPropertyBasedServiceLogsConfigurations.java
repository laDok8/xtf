...
/**
 * Implements {@link ServiceLogsSettings} in order to provide the concrete logic to handle Service
 * Logs Streaming configuration based on the {@code xtf.log.streaming.config} system property.
 */
public class SystemPropertyBasedServiceLogsConfigurations {
  ...
  private static final String ALLOWED_ATTRIBUTE_NAME_REGEX =
      String.format(
          "%s|%s|%s",
  ...
  // attributes values regex (which must allow for adding basically any char plus special ones, as
  // for providing
  // regex'es
  private static final String ALLOWED_ATTRIBUTE_VALUE_REGEX =
      "[\\\\|\\w|^|$|.|+|?|*|\\[|\\]|(|)|{|}|&|\\-|>|<|/|:]";
  // full attribute (name=value) regex
  private static final String ALLOWED_ATTRIBUTE_REGEX =
      "(?:"
          + ALLOWED_ATTRIBUTE_NAME_REGEX
          + ")"
  ...
  public SystemPropertyBasedServiceLogsConfigurations(String serviceLogsStreamingConfigProperty) {
    if ((serviceLogsStreamingConfigProperty == null)
        || serviceLogsStreamingConfigProperty.isEmpty()) {
      throw new IllegalArgumentException(
          "A valid configuration must be provided by setting \"xtf.log.streaming.config\" value, in"
              + " order to initialize a \"SystemPropertyBasedServiceLogsConfigurations\" instance");
    }
  ...
  private Map<String, ServiceLogsSettings> loadConfigurations() {
    return Stream.of(
            serviceLogsStreamingConfigProperty.split(CONFIGURATION_PROPERTY_ITEMS_SEPARATOR))
  ...
  private static ServiceLogsSettings getSettingsFromItemConfiguration(String configurationItem) {
    ...
    if (!configurationItem.matches(ALLOWED_ITEM_REGEX)) {
      throw new IllegalArgumentException(
          String.format(
              "The value of the \"xtf.log.streaming.config\" property items must match the"
                  + " following format: %s. Was: %s"
                  + ALLOWED_ITEM_REGEX,
              configurationItem));
    }
    // get all attributes for an item
    final String[] configurationAttributes =
        configurationItem.split(CONFIGURATION_PROPERTY_ATTRIBUTE_SEPARATOR);
  ...
  private static ServiceLogsSettings.Builder toBuilder(
    ...
    switch (attributeName) {
      ...
      default:
        throw new IllegalArgumentException(
            String.format(
                "Unconventional configuration attribute name: %s. Allowed configuration attributes"
                    + " names: (%s)",
