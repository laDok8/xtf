...
/**
 * Watches Pods on a <b>single</b> namespace and streams logs for all Pods containers to the
 * selected {@link PrintStream}.
 *
 * <p>Each log is prefixed with <code>namespace.pod-name.container-name</code> and colored
 * differently.
 */
@Slf4j
class PodLogsWatcher implements Watcher<Pod> {
  ...
  protected PodLogsWatcher(
      KubernetesClient client, String namespace, PrintStream printStream, Pattern filter) {
    this.client = client;
    this.namespace = namespace;
    // The life cycle of the PrintStream instance that is being used must be handled in the outer
    // scope, which is
    // not here.
  ...
  private void handleNewRunningContainers(Pod pod) {
    ...
    log.debug(
        ServiceLogUtils.getConventionallyPrefixedLogMessage(
            String.format(
                "existingContainersRunningStatuses.size=%s names=%s"
                    + " existingContainersRunningStatuses=%s",
    ...
    log.debug(
        ServiceLogUtils.getConventionallyPrefixedLogMessage(
            String.format(
                ...
                newContainersRunningStatuses.stream()
                    .map(cs -> cs.getName())
                    .collect(Collectors.joining()),
  ...
  private void handleNewTerminatedContainers(Pod pod) {
    ...
    log.debug(
        ServiceLogUtils.getConventionallyPrefixedLogMessage(
            String.format(
                "existingContainersTerminatedStatuses.size=%s names=%s"
                    + " existingContainersTerminatedStatuses=%s",
    ...
    log.debug(
        ServiceLogUtils.getConventionallyPrefixedLogMessage(
            String.format(
                "newContainersTerminatedStatuses.size=%s names=%s"
                    + " newContainersTerminatedStatuses=%s",
                newContainersTerminatedStatuses.size(),
                newContainersTerminatedStatuses.stream()
                    .map(cs -> cs.getName())
                    .collect(Collectors.joining()),
    ...
    for (ContainerStatus status : newContainersTerminatedStatuses) {
      ...
      if (logWatches.containsKey(status.getContainerID())) {
        ...
        log.debug(
            ServiceLogUtils.getConventionallyPrefixedLogMessage("Terminating PodLogsWatcher"));
  ...
  public void eventReceived(Action action, Pod pod) {
    log.debug(
        ServiceLogUtils.getConventionallyPrefixedLogMessage(
            String.format(
                "%s %s: %s", action.name(), pod.getMetadata().getName(), pod.getStatus())));
    switch (action) {
      case ADDED:
   ...
   *
   * @param before A list of {@link ContainerStatus} instances belonging to already existing
   *     containers
   * @param now A list of {@link ContainerStatus} instances belonging to new containers
   * @return A list of {@link ContainerStatus} instances representing the difference between those
   *     belonging to new containers and the existing ones, at a given moment in time.
   */
   ...
   * @return A list of containers running statuses related to containers belonging to the {@link
   *     Pod} instance
   */
