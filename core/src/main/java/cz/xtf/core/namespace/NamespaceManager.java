...
public class NamespaceManager {
   ...
   * returned by
   *
   * @see OpenShiftConfig#namespace(). If xtf.openshift.namespace.per.testcase=true then each entry
   *     points to namespace assigned to each test case by {@see
   ...
   * OpenShiftConfig#useNamespacePerTestCase()} is true.
   *
   ...
   *     for currently running test case when: -Dxtf.openshift.namespace.per.testcase=true. In case
   *     when current thread does not have associated test case (for example when initializing
   *     Openshift instance in static variable or static block) then {@link
   *     java.lang.RuntimeException} exception is thrown.
   */
  public static String getNamespace() {
    if (OpenShiftConfig.useNamespacePerTestCase()) {
      String namespace =
          NamespaceManager.getNamespaceForTestCase(TestCaseContext.getRunningTestCaseName());
      if (StringUtils.isEmpty(namespace)) {
        throw new RuntimeException(
            "There is no namespace associated with current thread or test case. This can happen in"
                + " case that OpenShift instance is created in static variable. In this case avoid"
                + " using static. Or in thread which is not associated with any test case.");
      }
      return namespace;
    } else {
      return OpenShiftConfig.namespace();
