    ...
    if (bm == null) {
      synchronized (BuildManager.class) {
        if (bm == null) {
          ...
          // If the build namespace is in a separate namespace to "master" namespace, we assume it
          ...
          if (!BuildManagerConfig.namespace().equals(OpenShiftConfig.namespace())) {
            ...
            try {
              ...
              log.warn(
                  "Attempt to add hard resource quota on {} namespace failed!", buildNamespace);
            }
          }
        }
      }
