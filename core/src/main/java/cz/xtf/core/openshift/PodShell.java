    ...
    public PodShellOutput execute(String... commands) {
        ...
        if (containerName == null) {
            openShift
                    .pods()
                    .withName(podName)
                    .writingOutput(baosOutput)
                    .writingError(baosError)
        ...
        new SimpleWaiter(execListener::hasExecutionFinished)
                .timeout(TimeUnit.MINUTES, 1)
                .reason("Waiting for " + Arrays.toString(commands) + " execution in '" + podName + "' pod.")
                .waitFor();
        try {
            new SimpleWaiter(() -> baosOutput.size() > 0 || baosError.size() > 0)
        ...
        return new PodShellOutput(
                baosOutput.toString().trim(), baosError.toString().trim());
    }
