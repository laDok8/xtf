...
public class OpenShiftWaiters {
   ...
   * @param failFast {@link BooleanSupplier} that returns true if waiter should fail due to error
   *     state of i.e. OpenShift
  ...
  /**
   * Creates waiter for latest build completion with a build timeout which is preconfigured to be up
   * to 10 minutes (but can also be customized by setting {@code xtf.waiting.build.timeout}), 5
   * seconds interval check and both logging points.
   *
  ...
  public Waiter hasBuildCompleted(String buildConfigName) {
    Supplier<String> supplier =
        () ->
            Optional.ofNullable(openShift.getLatestBuild(buildConfigName))
  ...
  /**
   * Creates waiter for build completion with a build timeout which is preconfigured to be up to 10
   * minutes (but can also be customized by setting {@code xtf.waiting.build.timeout}), 5 seconds
   * interval check and both logging points.
   *
  ...
  /**
   * Creates waiter for latest build presence with default timeout, 5 seconds interval check and
   * both logging points.
   *
  ...
  public Waiter isProjectClean() {
    return new SimpleWaiter(
            () -> {
              ...
              if (crdInstances == 0 & listRemovableResources.isEmpty()) {
                ...
                if (customResourceDefinitionList != null
                    && !customResourceDefinitionList.isEmpty()) {
        ...
        .onTimeout(
            () -> log.info("Cleaning namespace: " + openShift.getNamespace() + " - timed out."))
        .onFailure(() -> log.info("Cleaning namespace: " + openShift.getNamespace() + " - failed."))
        .onSuccess(
            () -> log.info("Cleaning namespace: " + openShift.getNamespace() + " - finished."))
        .failFast(failFast);
  }

  /**
   * Creates a waiter object waiting till all pods created by deployment config with name {@code
   * dcName} are ready. Tolerates any container restarts. Uses default timeout.
   *
  ...
  /**
   * Creates a waiter object that waits till all pods created by deployment config with name {@code
   * dcName} are ready. Tolerates {@code restartTolerace} container restarts. Uses default timeout.
   *
   * @param dcName name of deploymentConfig
  ...
  public Waiter isDeploymentReady(String dcName, int version, int restartTolerance) {
    ...
    String reason =
        "Waiting till all pods created by "
            + version
            + ". of "
            + dcName
  ...
  private Waiter isDeploymentReady(String dcName, Supplier<List<Pod>> ps, int restartTolerance) {
    ...
    Function<List<Pod>, Boolean> fc =
        ResourceFunctions.haveAnyPodRestartedAtLeastNTimes(restartTolerance);

    return new SupplierWaiter<>(ps, sc, fc).failFast(failFast);
  }

  /**
   * Creates a waiter that checks that exactly n pods is ready in project. Uses default timeout.
   * Tolerates any container restarts.
   *
  ...
  public Waiter areExactlyNPodsReady(int n, String key, String value) {
    ...
    String reason =
        "Waiting for exactly " + n + " pods with label " + key + "=" + value + " to be ready.";
  ...
  /**
   * Creates a waiter that checks that exactly n pods is running in project. Uses default timeout.
   * Tolerates any container restarts.
   *
   * @param n number of expected pods to wait upon
   * @return Waiter instance
  ...
  public Waiter areExactlyNPodsRunning(int n, String key, String value) {
    Supplier<List<Pod>> ps = () -> openShift.getLabeledPods(key, value);
    String reason =
        "Waiting for exactly " + n + " pods with label " + key + "=" + value + " to be running.";
  ...
  public Waiter havePodsBeenRestartedAtLeastNTimes(int times, String key, String value) {
    Supplier<List<Pod>> ps = () -> openShift.getLabeledPods(key, value);
    String reason =
        "Waiting for any pods with label "
            + key
            + "="
            + value
            + " having a restart count >= "
            + times
            + ".";
  ...
  private Waiter havePodsBeenRestartedAtLeastNTimes(int times, Supplier<List<Pod>> podSupplier) {
    return new SupplierWaiter<>(
            podSupplier, ResourceFunctions.haveAnyPodRestartedAtLeastNTimes(times))
        .failFast(failFast);
  }
}
