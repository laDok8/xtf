...
public class EventListFilter {
  ...
  /**
   * Filter events of types defined in the array (case insensitive). For example: {@code Warning},
   * {@code Normal}, ...
   */
  ...
  /**
   * Filter events with involved object kind defined in the array (case insensitive). For example:
   * {@code Warning}, {@code Normal}, ...
   */
  ...
  /**
   * Filter events that are last seen in any if given time windows. A structure of the array should
   * be: [from date], [until date], [from date] [until date],... Event needs to be seen strictly
   * after {@code from date} and before or at the same time as {@code until date}.
   *
   * <p>{@link ZonedDateTime} is used because a OpenShift cluster is distributed and time is
   * provided in {@link java.time.format.DateTimeFormatter#ISO_DATE_TIME} format that consider time
   * zones. Therefore wee need to compare it against {@link ZonedDateTime#now()} (for example)
   *
   * @see ZonedDateTime
   */
            ...
            event -> {
              if (event.getLastTimestamp() != null) {
                ZonedDateTime eventDate =
                    EventHelper.timestampToZonedDateTime(event.getLastTimestamp());
                for (int i = 0; i < dates.length - 1; i += 2) {
                  if (eventDate.isAfter(dates[i])
                      && (eventDate.compareTo(dates[i + 1]) == 0
                          || eventDate.isBefore(dates[i + 1]))) {
                    return true;
                  }
                }
              }
