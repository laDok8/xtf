...
/**
 * Loads properties stored in several different ways. Possible options, with top to down overriding
 * are: <br>
 *
 * <ul>
 *   <li><i>global-test.properties</i> on project root path
 *   <li><i>test.properties</i> on project root path - meant to be user specific, unshared
 *   <li><i>environment variables</i> in System.getEnv()
 *   <li><i>system properties</i> in System.getProperties()
 * </ul>
 */
@Slf4j
public final class XTFConfig {
  ...
  public static void loadConfig() {
    ...
    globalPropertiesPath =
        resolvePropertiesPath(
            System.getProperty(GLOBAL_TEST_PROPERTIES_PATH, "global-test.properties"));
    testPropertiesPath =
        resolvePropertiesPath(System.getProperty(TEST_PROPERTIES_PATH, "test.properties"));
    ...
    properties.putAll(
        System.getenv().entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> "xtf." + e.getKey().replaceAll("_", ".").toLowerCase(),
                    Map.Entry::getValue)));
