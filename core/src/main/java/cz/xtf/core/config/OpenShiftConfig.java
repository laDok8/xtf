...
public final class OpenShiftConfig {
  ...
  public static final String OPENSHIFT_NAMESPACE_PER_TESTCASE =
      "xtf.openshift.namespace.per.testcase";
  /**
   * Used only if xtf.openshift.namespace.per.testcase=true - this property can configure its
   * maximum length. This is useful in case where namespace is used in first part of URL of route
   * which must have <64 chars length.
   */
  public static final String OPENSHIFT_NAMESPACE_NAME_LENGTH_LIMIT =
      "xtf.openshift.namespace.per.testcase.length.limit";

  /**
   * Used only if xtf.openshift.namespace.per.testcase=true - this property configures default
   * maximum length of namespace name.
   */
  ...
  /**
   * Note that most likely you want to use {@see NamespaceManager#getNamespace()} which returns
   * actual namespace used by current tests. For example {@link OpenShifts#master()} is using {@see
   * NamespaceManager#getNamespace()} to get default namespace for currently running test.
   *
   ...
   *
   * @return limit on namespace if it's set by -Dxtf.openshift.namespace.per.testcase.length.limit
   *     property
   */
  public static int getNamespaceLengthLimitForUniqueNamespacePerTest() {
    return Integer.parseInt(
        XTFConfig.get(
            OPENSHIFT_NAMESPACE_NAME_LENGTH_LIMIT, DEFAULT_OPENSHIFT_NAMESPACE_NAME_LENGTH_LIMIT));
  }
   ...
   * https://mirror.openshift.com/pub/openshift-v4/x86_64/clients/ocp/ Channels are: stable, latest,
   * fast, candidate
   *
  ...
  /**
   * @return For backwards-compatibility reasons, also returns the value of xtf.openshift.token if
   *     xtf.openshift.master.token not specified
   */
