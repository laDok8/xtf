...
/**
 * The class provides support to create {@link FailFastCheck} {@see MultipleFailFastChecksHandler}
 * that checks if a waiter should fail due to an error state of cluster. The function is supposed to
 * be called by a {@link cz.xtf.core.waiting.Waiter}. If it returns true, the waiter should throw an
 * exception.
 *
 * <p>Example of use:
 *
 ...
 * </pre>
 *
 * <p>You can create more checks for events and every one of them will be checked. If one of them
 * returns true, the whole fail fast function returns true.
 */
