...
/**
 * Class provide support for exponential time backoff. This is convenient if you need to check some
 * event periodically. The class provides {@code next} method that implements wait (blocking or
 * non-blocking - returns true/false whether code can proceed).
 *
 * <p>Class wait time grow exponentially up to {@code maxBackoffMillis} time: 1sec, 2 sec, 4 sec, 8
...
public class ExponentialTimeBackoff {
  ...
  /**
   * Exponential backoff wait. If wait is blocking, {@link Thread#sleep} is called and true is
   * always returned. If wait is non-blocking, the method returns true if logic can proceed (enough
   * time has elapsed)
   *
