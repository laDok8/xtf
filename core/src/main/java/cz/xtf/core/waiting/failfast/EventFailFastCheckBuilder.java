...
/**
 * Builder for creating fail fast checks for event You can filter events by name, obj kind, obj
 * name, time,...
 *
 * <p>Builds {@link WatchedResourcesSupplier} and provides reason when a check fails - list of
 * filtered events and filter itself
 */
public class EventFailFastCheckBuilder {
  ...
  /**
   * Array of demanded types of events (case in-sensitive). One of them must be equal. For example:
  ...
  /**
   * Array of demanded object kinds of events (case in-sensitive). One of them must be equal. For
   * example: {@code persistentvolume}, {@code pod}, ...
   *
  ...
  public FailFastBuilder atLeastOneExists() {
    ...
    failFastBuilder.addFailFastCheck(
        new WatchedResourcesSupplier<>(
