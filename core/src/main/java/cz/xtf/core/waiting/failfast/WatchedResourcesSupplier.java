...
public class WatchedResourcesSupplier<X> implements FailFastCheck {
   ...
   * @param hasFailedFunction condition. The result is the result of {@link
   *     FailFastCheck#hasFailed()}
   * @param reasonFunction returns string as a reason why {@code hasFailed} returned {@code true}
   */
  WatchedResourcesSupplier(
      Supplier<X> resourceSupplier,
      Function<X, Boolean> hasFailedFunction,
