...
public class SimpleWaiter implements Waiter {
  ...
  public SimpleWaiter(
      BooleanSupplier successCondition, TimeUnit timeoutUnit, long timeout, String reason) {
  ...
  public boolean waitFor() {
    ...
    ExponentialTimeBackoff backoff =
        ExponentialTimeBackoff.builder().blocking(false).maxBackoff(32000).build();

    while (System.currentTimeMillis() < endTime) {
      if (backoff.next() && failFast.hasFailed()) {
        logPoint.logEnd(
            reason + " (fail fast method failure)", System.currentTimeMillis() - startTime, level);
