...
public class KnownIssueHandler implements TestExecutionExceptionHandler {
  ...
  public void handleTestExecutionException(ExtensionContext context, Throwable throwable)
      throws Throwable {
    String knownIssue =
        AnnotationSupport.findAnnotation(context.getElement(), KnownIssue.class)
            .orElseThrow(() -> throwable)
            .value();
    if (throwable instanceof SoftAssertionError
        && ((SoftAssertionError) throwable)
            .getErrors().stream().allMatch(e -> e.contains(knownIssue))) {
      throw new TestAbortedException(knownIssue, throwable);
    } else if (throwable instanceof AssertionError
        && !(throwable instanceof SoftAssertionError)
