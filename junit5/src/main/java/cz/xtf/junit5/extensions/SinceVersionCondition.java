...
public class SinceVersionCondition implements ExecutionCondition {
  ...
  public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
    SinceVersion sinceVersion =
        AnnotationSupport.findAnnotation(context.getElement(), SinceVersion.class).orElse(null);
    SinceVersions sinceVersions =
        AnnotationSupport.findAnnotation(context.getElement(), SinceVersions.class).orElse(null);
  ...
  public static ConditionEvaluationResult resolve(SinceVersion sinceVersion) {
    if (sinceVersion.name().equals("") == sinceVersion.imageMetadataLabelName().equals("")) {
      throw new RuntimeException(
          "Only one of 'name' and 'imageMetadataLabelName' can be presented in 'SkipFor'"
              + " annotation.");
    }
    ...
    if (!sinceVersion.name().equals("")) {
      ...
      matcher =
          Pattern.compile(sinceVersion.imageMetadataLabelName())
    ...
    if (matcher.matches()) {
      if (image.isVersionAtLeast(sinceVersion.since())) {
        return ConditionEvaluationResult.enabled(
            "'"
                + image.getRepo()
                + "' image tag is equal or bigger then expected. Tested feature should be"
                + " available.");
      } else {
        String jiraInfo =
            sinceVersion.jira().equals("") ? "" : " See " + sinceVersion.jira() + " for more info.";
        String message =
            "Tested feature isn't expected to be present in "
                + image.getRepo()
                + ":"
                + image.getTag()
                + "'. At least "
