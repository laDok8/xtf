...
public class SinceVersionCondition implements ExecutionCondition {
    ...
    public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
        SinceVersion sinceVersion = AnnotationSupport.findAnnotation(context.getElement(), SinceVersion.class)
                .orElse(null);
        SinceVersions sinceVersions = AnnotationSupport.findAnnotation(context.getElement(), SinceVersions.class)
                .orElse(null);
    ...
    public static ConditionEvaluationResult resolve(SinceVersion sinceVersion) {
        if (sinceVersion.name().equals("")
                == sinceVersion.imageMetadataLabelName().equals("")) {
        ...
        if (!sinceVersion.name().equals("")) {
            ...
            matcher = Pattern.compile(sinceVersion.imageMetadataLabelName())
        ...
        if (matcher.matches()) {
            if (image.isVersionAtLeast(sinceVersion.since())) {
                ...
                String jiraInfo =
                        sinceVersion.jira().equals("") ? "" : " See " + sinceVersion.jira() + " for more info.";
                String message = "Tested feature isn't expected to be present in " + image.getRepo() + ":"
                        + image.getTag() + "'. At least " + sinceVersion.since() + " tag is expected." + jiraInfo;
