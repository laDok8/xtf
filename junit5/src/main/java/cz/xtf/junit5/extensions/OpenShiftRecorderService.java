...
/**
 * Record OpenShift isolated state relative to a test. Provides an API and implementation for
 * setting filtered resources and recording their state upon request.
 *
 * <p>This service class can typically be used by classes that implement JUnit extension lifecycle
 * interfaces, e.g.: {@link org.junit.jupiter.api.extension.TestWatcher}, {@link
 * org.junit.jupiter.api.extension.BeforeAllCallback} etc.
 *
 * <p>One example of such classes is {@link OpenShiftRecorderHandler} which initializes (and
 * updates) filters or records OCP state when handling different JUnit events.
 *
 * <p>State recording is about downloading several resources logs from OCP.
 *
 * <p>Resources are filtered by name provided via {@link OpenShiftRecorder} annotation. Names are
 * turned into regexes by adding {@code .*} as a suffix. If no name is provided, resources in
 * namespaces (BM and master) are filtered automatically by recording which resources are seen
 * before test and so on.
 *
 * <p>Recorded resources:
 *
 * <ul>
 *   <li>pods states
 *   <li>deployment configs states
 *   <li>builds states
 *   <li>build configs states
 *   <li>image streams states
 *   <li>stateful sets states
 *   <li>routes states
 *   <li>services states
 *   <li>secrets states
 *   <li>logs of pods
 ...
 * </ul>
 *
 * <p>Use {@link JUnitConfig#recordDir()} ()} to set the directory of records.
 */
public class OpenShiftRecorderService {
  ...
  /**
   * Initialize filters by collecting information OCP resources which are relevant for the current
   * test execution context (e.g.: called by a {@link
   * org.junit.jupiter.api.extension.BeforeAllCallback#beforeAll(ExtensionContext)} implementation
   *
  ...
  public void initFilters(ExtensionContext context) {
    ...
    classStore.put(
        EVENT_FILTER_MASTER,
        new EventsFilterBuilder()
            .setExcludedUntil(ResourcesTimestampHelper.timeOfLastEvent(master)));
  ...
  /**
   * Update filters by adding information OCP resources which are relevant for the current test
   * execution context (e.g.: called by a {@link
   * org.junit.jupiter.api.extension.BeforeEachCallback#beforeEach(ExtensionContext)} implementation
   *
   * @param context The test execution context
   */
  public void updateFilters(ExtensionContext context) {
    ...
    if (!isFilterInitializationComplete(context)) {
      // set filter in a way everything created between filters initialization and now will be
      // captured - alwaysIncludedWindow
  ...
  private ExtensionContext.Store getClassStore(ExtensionContext extensionContext) {
    return extensionContext.getStore(
        ExtensionContext.Namespace.create(extensionContext.getRequiredTestClass()));
  }
  ...
  private <E extends HasMetadata> ResourcesFilterBuilder<E> getFilter(
      ExtensionContext context, String key) {
    OpenShiftRecorder classOpenShiftRecorder =
        AnnotationSupport.findAnnotation(context.getRequiredTestClass(), OpenShiftRecorder.class)
            .orElse(null);
    OpenShiftRecorder methodOpenShiftRecorder =
        AnnotationSupport.findAnnotation(context.getElement(), OpenShiftRecorder.class)
            .orElse(null);
    ...
    ResourcesFilterBuilder<E> filter =
        context.getTestMethod().isPresent()
            ? (ResourcesFilterBuilder<E>)
                getMethodStore(context).get(key, ResourcesFilterBuilder.class)
            : (ResourcesFilterBuilder<E>)
                getClassStore(context).get(key, ResourcesFilterBuilder.class);

    // OpenShiftRecorder (OpenShiftRecorderHandler) may be used in following ways
    // (1) A test is annotated by @OpenShiftRecorder - OpenShiftRecorder#resourcesNames is set to
    // default value -
    //     an array of one empty string.
    // (2) A test is annotated by @OpenShiftRecorder(string [string]*) -
    //     OpenShiftRecorder#resourcesNames is an array of strings. null value is not allowed in an
    // annotation.
  ...
  protected void saveStatefulSets(
      ExtensionContext context, ResourcesFilterBuilder<StatefulSet> masterFilter)
      throws IOException {
    final Path StatefulSetsLogPath =
        Paths.get(attachmentsDir(), dirNameForTest(context), "statefulSets.log");
  ...
  protected void saveISs(
      ...
      throws IOException {
    // master namespace
    final Path imageStreamsMasterLogPath =
        Paths.get(
            attachmentsDir(),
    ...
    // builds namespace (if not same)
    if (!isMasterAndBuildNamespaceSame()) {
      final Path imageStreamsBMLogPath =
          Paths.get(
              attachmentsDir(),
              dirNameForTest(context),
  ...
  protected void saveBCs(
      ...
      throws IOException {
    final Path bcMasterLogPath =
        Paths.get(
            attachmentsDir(),
            dirNameForTest(context),
            "buildConfigs-" + OpenShifts.master().getNamespace() + ".log");
    try (final ResourcesPrinterHelper<BuildConfig> printer =
        ResourcesPrinterHelper.forBCs(bcMasterLogPath)) {
    ...
    // builds namespace (if not same)
    if (!isMasterAndBuildNamespaceSame()) {
      final Path bcBMLogPath =
          Paths.get(
              attachmentsDir(),
              dirNameForTest(context),
              "buildConfigs-" + BuildManagers.get().openShift().getNamespace() + ".log");
      try (final ResourcesPrinterHelper<BuildConfig> printer =
          ResourcesPrinterHelper.forBCs(bcBMLogPath)) {
  ...
  protected void saveBuilds(
      ...
      throws IOException {
    // master namespace
    final Path buildsMasterLogPath =
        Paths.get(
            attachmentsDir(),
            dirNameForTest(context),
            "builds-" + OpenShifts.master().getNamespace() + ".log");
    try (final ResourcesPrinterHelper<Build> printer =
        ResourcesPrinterHelper.forBuilds(buildsMasterLogPath)) {
      OpenShifts.master().getBuilds().stream().filter(masterFilter.build()).forEach(printer::row);
    }
    // builds namespace (if not same)
    if (!isMasterAndBuildNamespaceSame()) {
      final Path buildsBMLogPath =
          Paths.get(
              attachmentsDir(),
              dirNameForTest(context),
              "builds-" + BuildManagers.get().openShift().getNamespace() + ".log");
      try (final ResourcesPrinterHelper<Build> printer =
          ResourcesPrinterHelper.forBuilds(buildsBMLogPath)) {
  ...
  protected void saveSecrets(ExtensionContext context) throws IOException {
    ...
    try (final ResourcesPrinterHelper<Secret> printer =
        ResourcesPrinterHelper.forSecrets(secretsLogPath)) {
      OpenShifts.master().getSecrets().forEach(printer::row);
    }
  }

  protected void saveServices(
      ExtensionContext context, ResourcesFilterBuilder<Service> masterFilter) throws IOException {
    final Path servicesLogPath =
        Paths.get(attachmentsDir(), dirNameForTest(context), "services.log");
    try (final ResourcesPrinterHelper<Service> printer =
        ResourcesPrinterHelper.forServices(servicesLogPath)) {
      OpenShifts.master().getServices().stream().filter(masterFilter.build()).forEach(printer::row);
    }
  }

  protected void saveRoutes(ExtensionContext context, ResourcesFilterBuilder<Route> masterFilter)
      throws IOException {
    final Path routesLogPath = Paths.get(attachmentsDir(), dirNameForTest(context), "routes.log");
    try (final ResourcesPrinterHelper<Route> printer =
        ResourcesPrinterHelper.forRoutes(routesLogPath)) {
      OpenShifts.master().getRoutes().stream().filter(masterFilter.build()).forEach(printer::row);
    }
  }

  protected void saveConfigMaps(
      ExtensionContext context, ResourcesFilterBuilder<ConfigMap> masterFilter) throws IOException {
    final Path configMapsLogPath =
        Paths.get(attachmentsDir(), dirNameForTest(context), "configMaps.log");
  ...
  protected void savePods(
      ...
      throws IOException {
    // master namespace
    final Path podsMasterLogPath =
        Paths.get(
            attachmentsDir(),
            dirNameForTest(context),
            "pods-" + OpenShifts.master().getNamespace() + ".log");
    try (final ResourcesPrinterHelper<Pod> printer =
        ResourcesPrinterHelper.forPods(podsMasterLogPath)) {
      OpenShifts.master().getPods().stream().filter(masterFilter.build()).forEach(printer::row);
    }
    // builds namespace (if not same)
    if (!isMasterAndBuildNamespaceSame()) {
      final Path podsBMLogPath =
          Paths.get(
              attachmentsDir(),
              dirNameForTest(context),
              "pods-" + BuildManagers.get().openShift().getNamespace() + ".log");
      try (final ResourcesPrinterHelper<Pod> printer =
          ResourcesPrinterHelper.forPods(podsBMLogPath)) {
  ...
  protected void saveDCs(
      ExtensionContext context, ResourcesFilterBuilder<DeploymentConfig> masterFilter)
      throws IOException {
    final Path dcsLogPath =
        Paths.get(attachmentsDir(), dirNameForTest(context), "deploymentConfigs.log");
    try (final ResourcesPrinterHelper<DeploymentConfig> printer =
        ResourcesPrinterHelper.forDCs(dcsLogPath)) {
  ...
  protected void saveEvents(
      ...
      throws IOException {
    // master namespace
    final Path eventsMasterLogPath =
        Paths.get(
            attachmentsDir(),
            dirNameForTest(context),
            "events-" + OpenShifts.master().getNamespace() + ".log");
    try (final ResourcesPrinterHelper<Event> printer =
        ResourcesPrinterHelper.forEvents(eventsMasterLogPath)) {
      OpenShifts.master().getEvents().stream().filter(masterFilter.build()).forEach(printer::row);
    }
    // builds namespace (if not same)
    if (!isMasterAndBuildNamespaceSame()) {
      final Path eventsBMLogPath =
          Paths.get(
              attachmentsDir(),
              dirNameForTest(context),
              "events-" + BuildManagers.get().openShift().getNamespace() + ".log");
      try (final ResourcesPrinterHelper<Event> printer =
          ResourcesPrinterHelper.forEvents(eventsBMLogPath)) {
  ...
  protected void saveBuildLogs(
      ...
      ResourcesFilterBuilder<Build> masterFilter,
      ResourcesFilterBuilder<Build> buildsFilter) {
    BiConsumer<OpenShift, ResourcesFilterBuilder<Build>> buildPrinter =
        (openShift, filter) ->
  ...
  private String attachmentsDir() {
    return JUnitConfig.recordDir() != null
        ? JUnitConfig.recordDir()
