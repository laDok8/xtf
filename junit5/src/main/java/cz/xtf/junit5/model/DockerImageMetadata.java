...
/**
 * Represents Docker image metadata by exposing convenience methods to access them and provides a
 * cache to optimize retrievals.
 */
public class DockerImageMetadata {
  ...
  private static final ConcurrentHashMap<String, DockerImageMetadata> CACHED_METADATA =
  ...
  public static DockerImageMetadata get(OpenShift openShift, Image image) {
    return CACHED_METADATA.computeIfAbsent(
        forgeMetadataKey(openShift, image), s -> getMetadata(openShift, image));
  }
  ...
  private static DockerImageMetadata getMetadata(OpenShift openShift, Image image) {
    ...
    ImageStreamTag imageStreamTag =
        openShift.getImageStreamTag(image.getRepo(), image.getMajorTag());
  ...
  /**
   * Returns default container command on Config/Cmd path If command consists of more parts, the
   * space-separated string of all those parts is returned.
   *
  ...
  /**
   * Returns integer set of exposed ports by specified protocol (eg. tcp, udp) on
   * Config/ExposedPorts path.
   *
   * @return port set
   */
