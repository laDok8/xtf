...
public class TestExecutionLogger implements TestExecutionListener {
  ...
  public void executionFinished(
    ...
    if (testIdentifier.isTest()) {
      String displayName = getTestDisplayName(testIdentifier);
      long executionTime =
          System.currentTimeMillis() - executionTimes.remove(testIdentifier.getUniqueId());
   ...
   * '[engine:test-engine]/[class:full.class.name.MyTest]/...' (May or may not continue. Eg.
   * parameterized tests).
   *
   * @param testIdentifier testIdentifier (isTest condition should be met, otherwise behaviour is
   *     undefined)
