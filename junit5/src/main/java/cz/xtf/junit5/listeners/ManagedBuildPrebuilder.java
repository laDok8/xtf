...
public class ManagedBuildPrebuilder implements TestExecutionListener {
  ...
  public void testPlanExecutionStarted(TestPlan testPlan) {
    ...
    // Attempting to start builds when they cannot start due to running pod limits cause random
    // timeouts,
    // so we try to be nice and wait until there are available capacity in the build manager
    // namespace.
    final OpenShift buildManagerOpenShift = OpenShifts.master(BuildManagerConfig.namespace());
    Waiter runningBuildsBelowCapacity =
        new SimpleWaiter(
                () ->
            ...
            .reason(
                "Waiting for a free capacity for running builds in "
                    + BuildManagerConfig.namespace()
                    + " namespace.");

    for (final BuildDefinition buildDefinition : buildsToBeBuilt) {
      // lazy creation, so that we don't attempt to create a buildmanager namespace when no builds
      // defined (e.g. OSO tests)
      ...
      try {
        ...
        // if the build failed, we need to treat the managed build as broken, better to delete it
        // (so that the test itself can try again)
  ...
  private void addBuildDefinition(
      List<BuildDefinition> buildsToBeBuilt,
      Set<BuildDefinition> buildsSeen,
                ...
                a -> {
                  try {
                    ...
                    if (result instanceof BuildDefinition) {
                      ...
                      log.error(
                          "Value present in {} is not instance of {}, not able to get ManagedBuild"
                              + " to be built",
                          result,
                    ...
                    log.error(
                        "Failed to invoke value() on annotation " + a.annotationType().getName(),
                        e);
                  }
                });
      }
  ...
  static boolean shouldSkipBuildsForClass(Class<?> klass) {
    boolean classDisabled =
        Arrays.stream(klass.getAnnotationsByType(Disabled.class)).findAny().isPresent();
    boolean classSkippedForStream =
        Arrays.stream(klass.getAnnotationsByType(SkipFor.class))
            .anyMatch(a -> SkipForCondition.resolve(a).isDisabled());
    boolean classSkippedForTestedVersion =
        Arrays.stream(klass.getAnnotationsByType(SinceVersion.class))
