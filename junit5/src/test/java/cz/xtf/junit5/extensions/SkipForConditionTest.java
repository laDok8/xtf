...
class SkipForConditionTest {
  ...
  @SkipFor(
      image = "eap",
      name = ".*eap-74.*",
      reason = "This test is skipped based on the image name.")
  class WellAnnotatedImageNameBasedSkipTest {}

  @SkipFor(
      image = "eap",
      imageMetadataLabelName = "centos7/s2i-base-centos7",
      reason = "This test is skipped based on the image docker labels")
  class WellAnnotatedImageMetadataLabelBasedSkipTest {}

  @SkipFor(
      image = "eap",
      imageMetadataLabelArchitecture = "s390x",
      reason = "This test is skipped based on the image metadata label architecture")
  class WellAnnotatedImageMetadataLabelArchitectureBasedSkipTest {}

  @SkipFor(
      image = "eap",
      subId = ".*74.*",
      reason = "This test is skipped based on the image product subId")
  ...
  @SkipFor(
      ...
      name = ".*eap-xp1.*",
      imageMetadataLabelName = "centos7/s2i-base-centos7",
      reason = "This test SHOULD BE skipped based on the image name.")
  ...
  @SkipFor(
      ...
      name = ".*eap-xp1.*",
      imageMetadataLabelName = "centos7/s2i-base-centos7",
      reason = "This test SHOULD BE skipped based on the image docker labels")
  ...
  @SkipFor(
      ...
      name = ".*eap-xp4.*",
      imageMetadataLabelArchitecture = "s390x",
      reason = "This test SHOULD BE skipped based on the image metadata label architecture")
  ...
  @SkipFor(
      ...
      imageMetadataLabelName = "centos7/s2i-base-centos7",
      subId = ".*74.*",
      reason = "This test SHOULD BE skipped based on the image product subId")
  class BadlyAnnotatedSubIdBasedSkipTest {}

  @Test
  void testUniqueCriteriaResolutionOnWellAnnotatedClasses() {
    Stream<Class> workingClasses =
        Stream.of(
            WellAnnotatedImageNameBasedSkipTest.class,
            // imageMetadataLabelName and imageMetadataLabelArchitecture try to fetch docker
            // metadata, so they require connection to openshift
    ...
    workingClasses.forEach(
        k -> {
          try {
            SkipForCondition.resolve(
                (SkipFor) Arrays.stream(k.getAnnotationsByType(SkipFor.class)).findFirst().get());
          } catch (RuntimeException re) {
            Assertions.fail(
                String.format(
                    "No exception is expected when resolving %s \"@SkipFor\" annotation",
                    k.getSimpleName()));
          }
        });
  }
          ...
          Exception exception =
              Assertions.assertThrows(
                  ...
                  () ->
                      SkipForCondition.resolve(
                          (SkipFor)
                              Arrays.stream(k.getAnnotationsByType(SkipFor.class))
                                  .findFirst()
                                  .get()));
          Assertions.assertEquals(
              "Only one of 'name', 'imageMetadataLabelName', 'imageMetadataLabelArchitecture' and"
                  + " 'subId' can be presented in 'SkipFor' annotation.",
  ...
  void testSubIdBasedSkipForResolution() {
    ConditionEvaluationResult conditionEvaluationResult =
        SkipForCondition.resolve(
            Arrays.stream(WellAnnotatedSubIdBasedSkipTest.class.getAnnotationsByType(SkipFor.class))
                .findFirst()
                .get());
    Assertions.assertTrue(
        conditionEvaluationResult.isDisabled(), "This test should be disabled via \"subId\"");
  }
}
