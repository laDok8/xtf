...
public class Http {
  ...
  public Http trustStore(
      Path trustStorePath, String trustStorePassword, HostnameVerifier verifier) {
    try {
      SSLContext sslContext =
          SSLContexts.custom()
      ...
      sslsf =
          new SSLConnectionSocketFactory(
              sslContext, new String[] {"TLSv1", "TLSv1.2"}, null, verifier);
  ...
  private CloseableHttpClient build() {
    ...
    if (preemptiveAuth && username != null && password != null) {
      // https://issues.apache.org/jira/browse/CODEC-89 Using this instead of encodeBase64String, as
      // some versions of apache-commons have chunking enabled by default
      String credentials =
          StringUtils.newStringUtf8(
              Base64.encodeBase64(
                  (username + ":" + password).getBytes(StandardCharsets.UTF_8), false));
